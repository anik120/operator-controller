...
Operator authors have the mechanisms to offer their product as part of a curated catalog of operators, that they can push updates to over-the-air (eg publish new versions, publish patched versions with CVEs, etc). Cluster admins can sign up to recieve these updates on clusters, by adding the catalog to the cluster. When a catalog is added to a cluster, the kubernetes extension packages (operators, or any other extension package) in that catalog become available on cluster for installation and receiving updates.  
...
To consume this catalog on cluster, create a `Catalog CR` with the image specified in the `spec.source.image` field: 
...
The packages made available for installation/recieving updates on cluster can then be explored by querying the `Packages` and `BundleMetadata` CRs: 
