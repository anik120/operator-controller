...
The packages available for installation/recieving updates on cluster can be explored by querying the `Packages` and `BundleMetadata` CRs: 
...
Individual `Pacakage`/`BundleMetadata` CRs can then be explored more by retriving their yamls. Eg the `operatorhubio-argocd-operator` CR has more detailed information about the `argocd-operator`: 
